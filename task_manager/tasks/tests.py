...
class TaskTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.valid_task_data = self.data.get("new_task")
        self.update_task_data = self.data.get("update_task")
...
class TaskViewsTest(TaskTestCase):
    ...
    def test_update_task(self):
        ...
        response = self.client.post(
            reverse("task_update", args=[id]), self.update_task_data
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse("login"))
        ...
        self.assertRedirects(response, reverse("tasks_list"))
        self.assertEqual(
            Task.objects.get(id=id).body,
            self.update_task_data.get("body"),
        )
    ...
    def test_delete_task(self):
        ...
        self.assertRedirects(response, reverse("tasks_list"))
        self.assertEqual(Task.objects.count(), self.task_count - 1)
        self.assertEqual(Task.objects.get(id=id).body, self.task1.body)
